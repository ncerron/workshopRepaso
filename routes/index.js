...
Router.get("/all/name/:asc", (req, res) => {
  const name_orden = req.params.asc;
  Propiedad.findAll({
    order: [["name", `${name_orden.toLocaleUpperCase()}`]],
  }).then((propiedades) => res.send(propiedades));
});

Router.get("/all/price/:asc", (req, res) => {
  const price_orden = req.params.asc;
  Propiedad.findAll({
    order: [["price", `${price_orden.toLocaleUpperCase()}`]],
  }).then((propiedades) => res.send(propiedades));
});
